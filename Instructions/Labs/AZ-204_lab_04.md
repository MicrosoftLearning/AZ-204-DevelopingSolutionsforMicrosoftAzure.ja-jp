...
ms.locfileid: "145995000"
---
# <a name="lab-04-develop-solutions-that-use-azure-cosmos-db"></a>ラボ 04: Azure Cosmos DB を使用するソリューションの開発
...
1.  ターミナル プロンプトから次のコマンドを実行して、Azure Cosmos DB .NET クライアント ライブラリを現在開いているプロジェクトに追加します。
    ...
    > **注**: **dotnet add package** コマンドを実行すると、**NuGet** から **Microsoft.Azure.Cosmos** パッケージが追加されます。 詳細については、「[Microsoft.Azure.Cosmos](https://www.nuget.org/packages/Microsoft.Azure.Cosmos/3.20.1)」を参照してください。
...
1.  18 行目で、空の文字列を **"/Category"** に置き換えて、 **PrimaryKey** の 値を設定します。
...
1.  **AdventureWorksCosmosContext.cs** ファイルのコード エディター タブで、次のコード行を追加して、参照先の **AdventureWorks.Models** プロジェクトから **AdventureWorks.Models** 名前空間をインポートします。
