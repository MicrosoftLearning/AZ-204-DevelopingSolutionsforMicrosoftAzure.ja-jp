...
1. **[Visual Studio Code]** ウィンドウの **[エクスプローラー]** ペインで、**bin\Debug\net8.0** ディレクトリに移動します。
    ...
    > **注**:**bin\Debug\net8.0** ディレクトリに移動したことを確認するには、Visual Studio Code の [ファイル] メニューで、[フォルダーを開く] を選択し、**Allfiles (F):\\Allfiles\\Labs\\11\\Starter\\Api\\bin\\Release\\net8.0\win-x86\publish** を参照してから、**[フォルダーの選択]** を選択します。
