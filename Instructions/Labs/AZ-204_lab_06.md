...
## ローカルのラボ環境の時刻が正しく設定されていることを確認します (タイムゾーンに従っていて、遅延がないこと)
...
このラボでは、Azure AD にアプリケーションを登録し、ユーザーを追加してから、アプリケーションへのユーザーのアクセスをテストして Azure AD でそれをセキュリティで保護できることを検証します。 また、Graph エクスプローラー ツールを使用して、Azure AD ユーザー アカウントの Graph API に対する要求をビルドしてテストします。

## アーキテクチャの図

![OpenID Connect、MSAL、および .NET SDK を使用して認証を行うユーザーを示すアーキテクチャ ダイアグラム。](./media/Lab06-Diagram.png)

### 演習 1: シングルテナント Azure AD 環境を構成する
