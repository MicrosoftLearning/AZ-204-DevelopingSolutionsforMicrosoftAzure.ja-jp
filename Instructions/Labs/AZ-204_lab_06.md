...
このラボでは、Microsoft Entra ID にアプリケーションを登録し、ユーザーを追加してから、アプリケーションへのユーザーのアクセスをテストして、Entra ID でアプリケーションのセキュリティが保護されていることを検証します。 また、Graph エクスプローラー ツールを使用して、Entra ID ユーザー アカウントの Graph API に対する要求をビルドおよびテストします。
