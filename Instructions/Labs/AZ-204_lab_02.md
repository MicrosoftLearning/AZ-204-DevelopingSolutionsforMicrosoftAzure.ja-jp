...
1. **[ストレージ アカウントを作成する]** ブレードの **[基本]** タブで、次のアクションを実行し、 **[確認および作成]** を選択します。
    ...
    | **[ストレージ アカウント名]** テキスト ボックス | 「**funcstor**_[yourname]_」と入力します。                              |
...
1. **[関数アプリの作成]** ブレードの **[基本]** タブで、次のアクションを実行してから、 **[次へ: ホスティング]** を選択します。
    ...
    | **[関数アプリ名]** テキスト ボックス   | 「**funclogic**_[yourname]_」と入力します。 |
    | **[公開]** セクション              | **[コード]** を選択します。                 |
    | **[ランタイム スタック]** ドロップダウン リスト | **[.NET]** を選択します。                |
    | **[バージョン]** ドロップダウン リスト       | **[3.1]** を選択します。                  |
...
1. **[ホスティング]** タブで、次のアクションを実行し、 **[確認および作成]** を選択します。
    ...
    | 設定                            | アクション                                             |
    | ---------------------------------- | -------------------------------------------------- |
    | **[ストレージ アカウント]** ドロップダウン リスト | **[funcstor** _[yourname]]_ ストレージ アカウントを選択します。 |

    次のスクリーンショットには、 **[関数アプリの作成]** ブレードの **[ホスティング]** タブで構成された設定が表示されています。

    ![[関数アプリの作成] ブレードの [ホスティング] タブで構成された設定を表示しているスクリーンショット ](./media/l02_create_a_function_app_hosting_tab.png)
...
1. **AzureWebJobsStorage** の値を、このラボで前に記録したストレージ アカウントの **接続文字列** に設定することによって更新します。
...
1. 開いているコマンド プロンプトが表示されたら、次のコマンドを実行して、**タイマー トリガー** テンプレートを使用して、**Azure Functions Core Tools** を使用して **Recurring** という名前の新しい関数を作成します。
...
1. 開いているコマンド プロンプトが表示されたら、次のコマンドを実行して、関数アプリ プロジェクトを実行します。
...
1. 開いているコマンド プロンプトが表示されたら、次のコマンドを実行して、**HTTP トリガー** テンプレートを使用して、**Azure Functions Core Tools** を使用して **GetSettingInfo** という名前の新しい関数を作成します。
...
#### <a name="task-3-register-azure-storage-blob-extensions"></a>タスク 3:Azure Storage Blob の拡張機能を登録する
...
1. 開いているコマンド プロンプトが表示されたら、次のコマンドを実行して、[Microsoft.Azure.WebJobs.Extensions.Storage](https://www.nuget.org/packages/Microsoft.Azure.WebJobs.Extensions.Storage/4.0.4) の拡張機能を登録します。
...
1. #### <a name="task-4-write-http-triggered-and-blob-inputted-function-code"></a>タスク 4:HTTP トリガーおよび BLOB 入力関数コードを記述する
...
1. タスク バーで、 **[Windows ターミナル]** アイコンを再び選択して、 **[Windows ターミナル]** アプリケーションの新しいインスタンスを開きます。
...
1. 開いているコマンド プロンプトが表示されたら、次のコマンドを実行して、Azure コマンド ライン インターフェイス (CLI) にログインします。
...
1. 次のコマンドを実行して、関数アプリ プロジェクトを公開します。
...
1. 表示されるポップアップ ダイアログで、次のアクションを実行します。 <!--Conan, as there is only sub step here, can we just combine the two as follows: In the popup dialog that appears, in the **HTTP method** list, select **GET**.-->
    - **[HTTP メソッド]** 一覧で、 **[GET]** を選択します。

1. **[Run]** を選択して、関数をテストします。

1. テストの実行の結果を確認します。 JSON コンテンツには、次のコードが含まれているはずです。
